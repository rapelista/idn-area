"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ProvincesModule = void 0;
const common_1 = require("@nestjs/common");
const provinces_service_1 = require("./provinces.service");
const provinces_controller_1 = require("./provinces.controller");
const typeorm_1 = require("@nestjs/typeorm");
const province_entity_1 = require("./entities/province.entity");
let ProvincesModule = class ProvincesModule {
};
exports.ProvincesModule = ProvincesModule;
exports.ProvincesModule = ProvincesModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([province_entity_1.Province])],
        controllers: [provinces_controller_1.ProvincesController],
        providers: [provinces_service_1.ProvincesService],
        exports: [provinces_service_1.ProvincesService],
    })
], ProvincesModule);
//# sourceMappingURL=provinces.module.js.map